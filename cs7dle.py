...
def evaluate_guess(secret_word, next_guess):
    result = []
    secret_word_scorecount = {c: secret_word.count(c) for c in set(secret_word)}
    for i in range(len(next_guess)):
        if next_guess[i] == secret_word[i]:
            result.append(MATCHES)
            secret_word_scorecount[next_guess[i]] -= 1
        else:
            if secret_word_scorecount.get(next_guess[i], 0) > 0:
...
def update_state(game_state, next_guess):
    scores = evaluate_guess(game_state["secret_word"], next_guess)
    game_state["guess_list"].append(next_guess)
    game_state["score_list"].append(scores)
    update_keyboard_state(game_state["keyboard_state"], next_guess, scores)
    if len(game_state["guess_list"]) == MAX_GUESSES:
        for i in range(len(score)):
            if score[i] == MATCHES for score in scores:
                game_state["stage"] = WIN
            else:
                game_state["stage"] = LOST
    game_state["seen_letters"].update(next_guess)
    """
...
def print_intro():
    ...
    cyan = set_color("Cyan", "cyan")
    grey = set_color("Grey", "grey")
    print(f"{green} letters are in the correct spot.")
    print(f"{cyan} letters are in the word, but in the wrong spot.")
